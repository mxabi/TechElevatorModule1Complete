...
//		}
		...
		System.out.println("What names start with 1?");
	...
	public static void stack_and_queue_demo(){

		//let's pretend that we're a browser keeping track of sites visited
		Stack<String> sites_visited = new Stack<>();
		sites_visited.push("www.techelevator.com");
		sites_visited.push("www.patriots.com");
		sites_visited.push("www.youtube.com");
		sites_visited.push("www.clubpenguin.com");

		//whats the last site? peek just looks
		String last = sites_visited.peek();//this will always be clubpenguin.com no matter how many times i call it
		last = sites_visited.peek();
		last = sites_visited.peek();
		last = sites_visited.peek();

		//pop will return and remove the last item
		String pop = sites_visited.pop(); //this will be club penguin
		pop = sites_visited.pop(); //this will be youtube

		while (sites_visited.size() > 0){
			//pop the next thing off and print it
			String popNext = sites_visited.pop();//pop decreases the size by 1 every time
			System.out.println(popNext);
		}
		//sites_visited.pop(); //EmptyStackException

		Queue<String> ticketMasterLine = new LinkedList<>();
		ticketMasterLine.offer("Jim");
		ticketMasterLine.offer("Abeda");
		ticketMasterLine.offer("Robert");
		ticketMasterLine.offer("Kevin");
		ticketMasterLine.offer("Yehudis");

		//now start giving people the tickets
		while(ticketMasterLine.size()>0){ //while there are still people in line
			String nowServing = ticketMasterLine.poll(); //gets who is next in line and removes from the line
