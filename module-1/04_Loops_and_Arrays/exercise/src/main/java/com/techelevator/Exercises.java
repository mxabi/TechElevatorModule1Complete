...
public class Exercises {
	...
	public int[] reverse3(int[] nums) {
		...
		if (nums.length <=3) {
			reversedArray[0] = nums[nums.length-1];
...
/*
	...
	public int countEvens(int[] nums) {
		...
		// check index to see if element of that index is divisible by 2
			if (nums[i] % 2 == 0){
				counter++;
			}
			// if divisble by 2 --> add to counter
		}
		return counter;
	}
	...
	public int sum13(int[] nums) {
		//Loop through array
		//For each element decide if it should be added to sum, and add it if i'm supposed to
		// if array has 13, return sum before 13 and one after 13
		//Will give you an out of bounds error if you start from i =0
		int sum = 0;
		if(nums[0] !=13){
			sum +=nums[0];
		}
	...
	public boolean has22(int[] nums) {
		//create variable to return, set a default value
		//return variable at the end
		//loop through array except last element
		//if element at index i and one after is 2 --> change has22 to true
		//check for out of bounds error, look through whole array except last element
		boolean has22 = false;
