...
public class ExceptionsLecture {
	/*
	  Two different kinds of errors
	    Compile time error - syntax error that the compiler sees and then refuses to execute
	    Run time error - occurs while the program is executing; IndexOutOfBounds exeception, NullReferenceException
	    Questionable code goes in the try
	    We can have multiple catches to catch each specific type of exception and/or generic Exceptino
	    We can have our code throw exceptions
	    We can create our own exceptions to throw
	    "Checked" exceptions must be handled (some FileIO or if you specifically throw an exception) by either
	       rethrowing by addign throws bla to method signature or catching and handling
	    try {
	       do something
	    } catch (Exception e){
	      handle the excecption
	     } finally {
	        this block executes regardless of if there was a exception or not
	     }
	     this code exeutes too regardless of try or catch
	 */
	...
	public static void main(String[] args) {
		...
		//i can tell if there was an excpetion or not based onthe bool

		Scanner scan = new Scanner(System.in);//attach ONE scanner to system.in
		...
		try {
			...
			//int myInt = Integer.parseInt("not an int"); //this line will throw a NumberFormatException that won't get caught
		...
		try {
			System.out.println(cities[0]);
			System.out.println(cities[1]);
			int myInt = Integer.parseInt("not an int"); //this line will throw an exception
			System.out.println(cities[2]);
			System.out.println(cities[3]);  // This statement will throw an ArrayIndexOutOfBoundsException
			System.out.println("are all in Ohio."); // This line won't execute because the previous statement throws an Exception
		} catch(Exception e) {
