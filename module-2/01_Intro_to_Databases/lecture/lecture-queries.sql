...
--Data in a relational database can be accessed and assembled in different ways
...
--SELECT commana delimeted column list
...
SELECT * FROM park; -- not a huge fan of using *, just select by column name and what you need
...
SELECT city_name, state_abbreviation
FROM city
WHERE state_abbreviation != 'CA'; -- or state abbreviation <> 'CA';
...
--between is inclusive, equiv to WHERE area >=100 and area <=200
-- DATES
